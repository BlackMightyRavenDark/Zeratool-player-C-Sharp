    ...
    public class ZeratoolPlayerEngine : Control
    {
        ...
        private int ConnectMediaSplitter_Manual(IPin sourcePinOut)
        {
            ...
            if (splitterId >= 0)
            {
                ...
                errorCode = CreateDirectShowFilter(filterItem.Guid, out mediaSplitter);
                if (errorCode != S_OK)
                {
        ...
        private int RenderVideoStream_Manual(IPin pinOut)
        {
            ...
            if (filters.videoDecoderId >= 0)
            {
                ...
                errorCode = CreateDirectShowFilter(filterItem.Guid, out videoDecoder);
                if (errorCode != S_OK)
                {
                    AddToLog($"Loading {filterItem.DisplayName}", ErrorCodeToString(errorCode));
                    return errorCode;
            ...
            errorCode = CreateDirectShowFilter(filterItem.Guid, out videoRenderer);
            if (errorCode != S_OK)
            {
                AddToLog($"Loading {filterItem.DisplayName}", ErrorCodeToString(errorCode));
        ...
        private int FindAndConnectVideoDecoder_Manual(IPin pinOut)
        {
            ...
            {
                int errorCode = CreateDirectShowFilter(item.Guid, out IBaseFilter filter);
                if (errorCode != S_OK)
                {
        ...
        private int RenderAudioStream_Manual(IPin splitterPinOut)
        {
            ...
            if (filters.audioDecoderId >= 0)
            {
                ...
                errorCode = CreateDirectShowFilter(filterItemAudioDecoder.Guid, out audioDecoder);
                if (errorCode != S_OK)
                {
        ...
        { 
            ...
            int errorCode = CreateDirectShowFilter(filterItem.Guid, out videoRenderer);
            if (errorCode != S_OK)
            {
                AddToLog($"Loading {filterItem.DisplayName}", ErrorCodeToString(errorCode));
                return errorCode;
        ...
        private int RenderAudioStream_Intellectual()
        {
            ...
            errorCode = CreateDirectShowFilter(filterItem.Guid, out audioDecoder);
            if (errorCode != S_OK)
            {
                AddToLog($"Loading {filterItem.DisplayName}", ErrorCodeToString(errorCode));
                graphBuilder.RemoveFilter(audioRenderer);
